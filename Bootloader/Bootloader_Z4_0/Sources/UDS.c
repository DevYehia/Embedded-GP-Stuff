...
void UDS_Security_Access(void){
    ...
    static SECURITY_ACCESS_STATE currState = SEED_STATE; //Current state of Security Access Request

    //if security access seed request is of incorrect length
	if (currState = SEED_STATE && requestFrame.dataSize != SEC_ACCESS_SEED_REQ_LEN){
        UDS_Create_neg_response(WRONG_MSG_LEN_OR_FORMAT, READY);
        return;
    }

    //if security access key request is of incorrect length
	if (currState = KEY_STATE && requestFrame.dataSize != SEC_ACCESS_KEY_REQ_LEN){
        UDS_Create_neg_response(WRONG_MSG_LEN_OR_FORMAT, READY);
        return;
    }
    ...
    //incorrect sequence
    if(currState == SEED_STATE && req_type == KEY_REQ){
